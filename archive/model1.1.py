...
def flatten_license_plate(image, x1, y1, x2, y2):
    """Apply perspective transformation to flatten the license plate"""
    ...
    plate_small = imutils.resize(plate_img, height=300)
    
    # Convert to grayscale
    gray = cv2.cvtColor(plate_small, cv2.COLOR_BGR2GRAY)
    ...
    for c in cnts:
        ...
        warped = four_point_transform(plate_small, box)
        break  # only use the largest valid contour
    ...
    # warped = cv2.resize(warped, (plate_img.shape[1], plate_img.shape[0]))
...
def split_license_plate(image, x1, y1, x2, y2):
    """Split the license plate into two lines for square plates"""
    plate_img = image[y1:y2, x1:x2]
    height = y2 - y1
...
def process_license_plate(image, x1, y1, x2, y2):
    ...
    # Apply perspective transformation to flatten the plate
    flattened_plate = flatten_license_plate(image, x1, y1, x2, y2)
    
    if flattened_plate is None:
        return "", 0
    
    # Use the flattened plate image instead of cropping directly from the original
    ...
    if plate_type == "rectangle":
        ...
        # For square plates, use the height of the flattened plate to split
        height, width = flattened_plate.shape[:2]
        line1 = flattened_plate[0:height // 2, :]
        line2 = flattened_plate[height // 2:, :]
